...
class My_Controller extends CI_Controller {
    ...
    public function __construct() {
        ...
        $rolesid_roles = $this->db->get_where('roles', array('status'=> 1, 'school_id'=>$this->session->userdata('school_id')))->result();
        ...
        if (count($rolesid_roles) > 0) {
            foreach ($rolesid_roles as $key => $value) {

               
                if ($value->slug == 'student'.$this->session->userdata('school_id')) {
                    define('STUDENT', $value->id);
                }

                if ($value->slug == 'guardian'.$this->session->userdata('school_id')) {
                    define('GUARDIAN', $value->id);
                }
        
                if ($value->slug == 'teacher'.$this->session->userdata('school_id')) {
                    define('TEACHER', $value->id);
                }
        
                if ($value->slug == 'admin'.$this->session->userdata('school_id')) {
                    define('ADMIN', $value->id);
                }
            }           
        }
