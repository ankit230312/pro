    ...
    public function login() {
        ...
        if ($_POST) {
            ...
            if (!empty($login)) {
                ...
                $rolesid_roles = $this->db->get_where('roles', array('status'=> 1, 'id'=>$login->role_id))->row();

             
              

                if ($rolesid_roles) {
                    if ($rolesid_roles->slug == 'student'.$login->school_id) {
                        define('STUDENT', $rolesid_roles->id);
                    }else{
                        define('STUDENT', '4');
        
                    }
        
                    if ($rolesid_roles->slug == 'guardian'.$login->school_id) {
                        define('GUARDIAN', $rolesid_roles->id);
                    }else{
                        define('GUARDIAN', '3');
        
                    }
            
                    if ($rolesid_roles->slug == 'teacher'.$login->school_id) {
                        define('TEACHER', $rolesid_roles->id);
                    }else{
                        define('TEACHER', '5');
        
                    }
            
                    if ($rolesid_roles->slug == 'admin'.$login->school_id) {
                         define('ADMIN', $rolesid_roles->id);
                    }else{

                        if ($rolesid_roles->slug == 'subadmin0') {
                            define('ADMIN', $rolesid_roles->id);                           
                        }else{
                            define('ADMIN', '2');            
                        }                        
                ...
                // check school status                
                if($login->role_id != SUPER_ADMIN && $rolesid_roles->slug != 'subadmin0'){                    
                ...
                }elseif ($rolesid_roles->slug == 'subadmin0') {
                    $profile = $this->auth->get_single('system_admin', array('user_id' => $login->id));
