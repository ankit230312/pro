<?php 
        ...
        foreach ($certificateex as $key => $value) {

            $certificateex_single  = explode('_', $value);



             $school_id = $certificateex_single[0];
             $student_id = $certificateex_single[1];
              $class_id = $certificateex_single[2];
              $certificate_id = $certificateex_single[3];
            ...
            $certificate = $this->type->get_single('certificates', array('id' => $certificate_id));
            $school = $this->type->get_single('schools', array('id'=>$certificate->school_id, 'status'=>1));
            $student = $this->type->get_student($student_id, $class_id, $school->academic_year_id);     
            $certificate->main_text = get_formatted_certificate_text($certificate->main_text, $student->role_id, $student->user_id);

            create_log('Has been generate a certificate for student : '.$student->name); 
    ...
    <body>
        <div class="x_content">
            ...
            <!-- this row will not appear when printing -->
                <div class="col-xs-12">
                <div class="generatedby"> Generated By Educlouds</div>
                </div>
