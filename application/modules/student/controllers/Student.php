...
class Student extends MY_Controller {
    ...
    private function _get_posted_student_data() {
        ...
        if ($this->input->post('id')) {
            ...
            if($this->input->post('is_guardian') == 'exist_guardian'){
                ...
                $guardianrole_id = '';
                $guardiansid = 'guardian'.$data['school_id'];
                $rolesguardian  = $this->db->get_where('roles', ['slug'=> $guardiansid, 'school_id'=>$data['school_id']])->row(); 
                if($rolesguardian){
                    $guardianrole_id = $rolesguardian->id;
                }

                
                $info['role_id']  = $guardianrole_id;
        ...
        if ($_FILES['other_documents']['name'][0]) {
            $data['other_documents'] = $this->_upload_other_documents();
        }
    ...
    * ********************************************************** */
    private function _upload_transfer_certificate() {
        ...
        /*****************Function _upload_other_documents**********************************
    * @type            : Function
    * @function name   : _upload_other_documents
    * @description     : process to upload student transfer_certificate in the server
    *                     and return photo file name
    * @param           : null
    * @return          : $return_photo string value
    * ********************************************************** */
    private function _upload_other_documents() {

        $prev_other_documents = $this->input->post('prev_other_documents');
        $other_documents = $_FILES['other_documents']['name'][0];
        
        $return_other_documents = [];
        if ($other_documents != "") {

            foreach ($_FILES['other_documents']['name'] as $key => $value) {

                $other_documents_type = $_FILES['other_documents']['type'][$key];

                if ($other_documents_type == 'image/jpeg' || $other_documents_type == 'image/pjpeg' ||
                        $other_documents_type == 'image/jpg' || $other_documents_type == 'image/png' ||
                        $other_documents_type == 'image/x-png' || $other_documents_type == 'image/gif') {

                    $destination = 'assets/uploads/student-other-document/';
                    $other_documents = $_FILES['other_documents']['name'][$key];
                    $file_type = explode(".", $other_documents);
                    $extension = strtolower($file_type[count($file_type) - 1]);
                    $other_documents_path = 'other-' . time() .  rand(11111,99999999) .'-sms.' . $extension;

                    move_uploaded_file($_FILES['other_documents']['tmp_name'][$key], $destination . $other_documents_path);

                    // need to unlink previous other_documents
                    // if ($prev_other_documents != "") {
                    //     if (file_exists($destination . $prev_other_documents)) {
                    //         @unlink($destination . $prev_other_documents);
                    //     }
                    // }

                    array_push($return_other_documents, $other_documents_path);
                    //$return_other_documents = $other_documents_path;
                }
            }
          
        } else {
            $return_other_documents = $return_other_documents;
        }

        return json_encode($return_other_documents);
    }
