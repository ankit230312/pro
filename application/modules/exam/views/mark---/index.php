                 ...
                 <?php echo form_open(site_url('exam/mark/add'), array('name' => 'addmark', 'id' => 'addmark', 'class'=>'form-horizontal form-label-left'), ''); ?>
                  ...
                  <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                    ...
                    <tbody id="fn_mark">   
                        <?php
                        $count = 1;
                        if (isset($students) && !empty($students) && !empty($exam_main)) {
                            ?>
                            <?php foreach ($students as $obj) { ?>
                            <?php  $mark = get_exam_mark($school_id, $obj->student_id, $academic_year_id, $exammain_id, $class_id, $section_id, $subject_id); ?>
                            <?php  $attendance = get_exam_attendance($school_id, $obj->student_id, $academic_year_id, $exammain_id, $class_id, $section_id, $subject_id); ?>
                                <tr>
                            ...
                            <?php } ?>
                        <?php }elseif (isset($students) && !empty($students) && !empty($online_exam_main)) {
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <input type="number" id="written_mark_<?php echo $obj->id; ?>" itemid="<?php echo $obj->id; ?>" value="<?php if(!empty($obj) && $obj->written_mark > 0){ echo $obj->written_mark; }else{ echo '';} ?>"  name="written_mark[<?php echo $obj->id; ?>]" class="form-control form-mark col-md-7 col-xs-12 fn_mark_total" required="required"  autocomplete="off"/>
                                    </td>
                                    <td>
                                            <input type="number"  id="written_obtain_<?php echo $obj->id; ?>"  itemid="<?php echo $obj->id; ?>"  value="<?php if(!empty($obj) && $obj->written_obtain > 0 ){ echo $obj->written_obtain; }else{ echo '0'; } ?>"  name="written_obtain[<?php echo $obj->id; ?>]" class="form-control form-mark col-md-7 col-xs-12 fn_mark_total"   autocomplete="off"/>
                                        
                                    </td>
                                    
                                    <td>
                                        <input type="number"  id="tutorial_mark_<?php echo $obj->id; ?>" itemid="<?php echo $obj->id; ?>"  value="<?php if(!empty($obj) && $obj->tutorial_mark > 0){ echo $obj->tutorial_mark; }else{ echo '';} ?>"  name="tutorial_mark[<?php echo $obj->id; ?>]" class="form-control form-mark col-md-7 col-xs-12 fn_mark_total"   autocomplete="off"/>
                                    </td>
                                    <td>
                                        <input type="number"  id="tutorial_obtain_<?php echo $obj->id; ?>" itemid="<?php echo $obj->id; ?>"   value="<?php if(!empty($obj) && $obj->tutorial_obtain > 0 ){ echo $obj->tutorial_obtain; }else{ echo ''; } ?>"  name="tutorial_obtain[<?php echo $obj->id; ?>]" class="form-control form-mark col-md-7 col-xs-12 fn_mark_total"  autocomplete="off"/>

                                    </td>
                                    
                                    <td>
                                        <input type="number"  id="practical_mark_<?php echo $obj->id; ?>" itemid="<?php echo $obj->id; ?>"  value="<?php if(!empty($obj) && $obj->practical_mark > 0){ echo $obj->practical_mark; }else{ echo '';} ?>"  name="practical_mark[<?php echo $obj->id; ?>]" class="form-control col-md-7 form-mark col-xs-12 fn_mark_total"   autocomplete="off"/>
                                    </td>
                                    <td>
                                        <input type="number"  id="practical_obtain_<?php echo $obj->id; ?>" itemid="<?php echo $obj->id; ?>"   value="<?php if(!empty($obj) && $obj->practical_obtain > 0 ){ echo $obj->practical_obtain; }else{ echo ''; } ?>"  name="practical_obtain[<?php echo $obj->id; ?>]" class="form-control form-mark col-md-7 col-xs-12 fn_mark_total"   autocomplete="off"/>

                                    </td>
                                    
                                    <td>
                                        <input type="number"  id="viva_mark_<?php echo $obj->id; ?>" itemid="<?php echo $obj->id; ?>"  value="<?php if(!empty($obj) && $obj->viva_mark > 0){ echo $obj->viva_mark; }else{ echo '';} ?>"  name="viva_mark[<?php echo $obj->id; ?>]" class="form-control form-mark col-md-7 col-xs-12 fn_mark_total"  autocomplete="off"/>
                                    </td>
                                    <td>
                                    <input type="number"  id="viva_obtain_<?php echo $obj->id; ?>" itemid="<?php echo $obj->id; ?>"  value="<?php if(!empty($obj) && $obj->viva_obtain > 0 ){ echo $obj->viva_obtain; }else{ echo ''; } ?>"  name="viva_obtain[<?php echo $obj->id; ?>]" class="form-control form-mark col-md-7 col-xs-12 fn_mark_total"   autocomplete="off"/>

                                    </td>                                    
                                    <td>
                                        <input type="number"  id="exam_total_mark_<?php echo $obj->id; ?>" value="<?php if(!empty($obj) && $obj->total_mark > 0){ echo $obj->total_mark; }else{ echo '0';} ?>"  name="exam_total_mark[<?php echo $obj->id; ?>]" class="form-control form-mark col-md-7 col-xs-12"  autocomplete="off" />
                                    </td>
                                    <td>
                                            <input type="number"  id="obtain_total_mark_<?php echo $obj->id; ?>" value="<?php if(!empty($obj) && $obj->total_obtain_mark > 0 ){ echo $obj->total_obtain_mark; }else{ echo '0'; } ?>"  name="obtain_total_mark[<?php echo $obj->id; ?>]" class="form-control form-mark col-md-7 col-xs-12"  autocomplete="off"/>
                                    ...
                                    <td>
                                        <select  class="form-control col-md-7 col-xs-12" name="grade_id[<?php echo $obj->id; ?>]"  required="required">                                
                                            <option value="">--<?php echo $this->lang->line('select'); ?>--</option>
                                             <?php foreach ($grades as $grade) { ?>
                                            <option value="<?php echo $grade->id; ?>" <?php if(isset($obj) && $obj->grade_id == $grade->id){ echo 'selected="selected"';} ?>><?php echo $grade->name; ?> [<?php echo $grade->point; ?>]</option>
                                            <?php } ?>
                                        </select>
                                    </td>
                                    <td>
                                        <?php if(!empty($attendance)){ ?>
                                            <input type="text"  id="remark_<?php echo $obj->id; ?>" value="<?php if(!empty($obj) && $obj->remark != '' ){ echo $obj->remark; }else{ echo ''; } ?>"  name="remark[<?php echo $obj->id; ?>]" class="form-control form-mark col-md-7 col-xs-12"  autocomplete="off"/>
                                        <?php }else{ ?>
                                            <input readonly="readonly" type="text" value=""  name="remark[<?php echo $obj->id; ?>]" class="form-control form-mark col-md-7 col-xs-12"   autocomplete="off"/>
                                        <?php } ?>
                                    </td>
                                </tr>
                            <?php } ?>
                ...
                <div class="form-group">
                    <div class="col-md-6 col-md-offset-5">
                        <?php  if (isset($students) && !empty($students)) { ?>
                                ...
                                <input type="hidden" value="<?php echo $school_id; ?>"  name="school_id" />
                                <input type="hidden" value="<?php echo $online_exam_main; ?>"  name="online_exam_main" />
                                <input type="hidden" value="<?php echo $exam_main; ?>"  name="exam_main"/>
