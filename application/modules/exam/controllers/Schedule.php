...
class Schedule extends MY_Controller {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($insert_id) {
                     ...
                     $get_user_list =  $this->schedule->get_user_list($data['school_id'], $data['class_id'], $data['section_id'], $school->academic_year_id);
                     foreach($get_user_list as $keys=> $vls){
                         notifications($vls->student_id, 'student', 'offline_exams', 'Has been created an Offline exam : '.$data['title']);
                     }
    ...
    * ********************************************************** */
    public function edit($id = null) {
        ...
        $this->data['exam_id'] = $this->data['schedule']->exam_id;
    ...
    public function question_order()
    {
        if ($_POST) {
            ...
            if($question_id && $ordernumber && is_array($this->input->post('question_id'))){

                foreach ($this->input->post('question_id') as $key => $value) {
                    $dataupdate['order'] = $ordernumber[$key];
                    $this->db->where('school_id',$school_id);
                    $this->db->where('exam_id',$exam_id);
                    $this->db->where('question_id',$value);
                    $this->db->update('exam_offline_to_questions', $dataupdate);
                }
            }
           echo "1";
    ...
    * ********************************************************** */
    ...
    public function addquestion($exam_id = null) {
        ...
        if(isset($exam_id) && !is_numeric($exam_id)){
            error($this->lang->line('unexpected_error'));
            redirect('exam/index');
        }
      
        
        $this->data['exam'] = $this->schedule->get_single_schedule($exam_id); 

        
        $school_id = $this->data['exam']->school_id;
        $class_id = $this->data['exam']->class_id;
        $subject_id = $this->data['exam']->subject_id;        
        $academic_year_id = $this->data['exam']->academic_year_id;        
                
        $this->data['exams_list'] = $this->schedule->get_schedule_list($school_id, $class_id, $subject_id, $academic_year_id);
        
        // Exam question class wise
        $this->data['questions'] =  $this->schedule->get_question_list($school_id, $class_id, $subject_id);
        
       
        // Exam wise question id's
        $this->data['exam_questions'] =  $this->schedule->get_list('exam_offline_to_questions', array('status'=>1, 'exam_id'=>$exam_id), '','', '', 'order', 'ASC');
        ...
        $this->layout->view('exam/schedule/add_question', $this->data);
    ...
    public function add_question_to_exam(){
       ...
       $exist = $this->schedule->get_single('exam_offline_to_questions', array('school_id'=>$school_id, 'exam_id' => $exam_id, 'question_id'=>$question_id));
       ...
       if(empty($exist)){
            ...
            $insert_id = $this->schedule->insert('exam_offline_to_questions', $data);
            
            // now making html for show data
            if($insert_id){
               
                $exam_questions = $this->schedule->get_list('exam_offline_to_questions', array('status'=>1, 'exam_id'=>$exam_id), '','', '', 'order', 'ASC');
               
                $str =  get_question_detail($exam_id, $question_id, count($exam_questions));
    ...
    public function remove_question_from_exam(){
        ...
        $exam_id = $this->input->post('exam_id');
        $question_id =  $this->input->post('question_id');
        if($this->schedule->delete('exam_offline_to_questions', array('school_id'=>$school_id, 'exam_id' => $exam_id, 'question_id'=>$question_id))){
            echo TRUE;
        }else{
            echo FALSE;
        }
        ...
        // Ajax call
    public function get_exam_by_subject() {
        ...
        $exams = $this->data['exams'] = $this->exam->get_exam_list($school_id, $class_id, $subject_id, $academic_year_id);
