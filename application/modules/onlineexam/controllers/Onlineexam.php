...
class Onlineexam extends MY_Controller {
    ...
    * ********************************************************** */
    public function index($class_id = null) {
        ...
        $this->data['exam_type_id'] = '';
        $this->data['exam_sub_type_id'] = '';
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($insert_id) {
                    ...
                    $get_user_list =  $this->online_exam->get_user_list($data['school_id'], $data['class_id'], $data['section_id'], $school->academic_year_id);
                    foreach($get_user_list as $keys=> $vls){
                        notifications($vls->student_id, 'student', 'online_exams', 'Has been created an Online exam');
                    }
    ...
    * ********************************************************** */
    public function edit($id = null) {
        ...
        $exam_type_id = $this->data['online_exam']->exam_type_id;        
        $exam_id = $this->data['online_exam']->title;        
        $exam_sub_type_id = $this->data['online_exam']->exam_sub_type_id;        
    ...
    public function invigilate_capture($id = null) {
        ...
        if($this->session->userdata('role_id') == STUDENT || $this->session->userdata('role_id') == GUARDIAN){
            error($this->lang->line('unexpected_error'));
            redirect('onlineexam/index');
        }
        ...
        $allstudents = $this->online_exam->get_student_list_for_invigilate($school_id, $academic_year_id, $class_id, $subject_id, $id);
        ...
        $this->layout->title($this->lang->line('edit') . ' | ' . SMS);
        $this->layout->view('online_exam/invigilate_capture', $this->data);
    }



    public function get_image_by_online_exam()
    {
        ...
        $this->data['exam_taken_exams'] = $this->db->order_by('id', 'desc')->limit(200)->get_where('webcam_upload', ['profile_id'=> $student_id, 'school_id'=> $school_id, 'academic_year_id'=> $academic_year_id, 'online_exam_id'=> $exam_id])->result();
         $this->load->view('take_exam/get-single-images-invigilate-capture', $this->data);
    }
