...
class Question extends MY_Controller {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                if ($insert_id) {                    
                    ...
                    $this->__save_question_option($insert_id);
                       if ($_POST['maintype'] == 'singlepage') {
                            $this->data['questions'] = $this->question->get_question_list($data['school_id'], $data['class_id'], $data['subject_id']);
                    
                            $questionhtml =   $this->load->view('online_exam/get_ajax_question_list', $this->data, true);  
                            echo  json_encode(['status'=>1, 'question_id'=> $insert_id, 'html'=> $questionhtml]);
                            die;
    ...
    public function question_addoffline() {
        ...
        if($question_type == 'single'){
            ...
            for($i = 1; $i <= $total_option; $i++){
               ...
               $str .= '<div class="item form-group">
                   <label class="control-label col-md-3 col-sm-3 col-xs-12 disss-labbe" for="option">'. $this->lang->line('option').' '.($i+$key).' <span class="required">*</span></label>
            ...
            for($i = 1; $i <= $total_option; $i++){
               ...
               $str .= '<div class="item form-group">
                   <label class="control-label col-md-3 col-sm-3 col-xs-12 disss-labbe" for="option">'. $this->lang->line('answer').' '.($i+$key).' <span class="required">*</span></label>
            ...
            $str .= '<div class="item form-group">
                <label class="control-label col-md-3 col-sm-3 col-xs-12 disss-labbe" for="option"  style="padding-top:0px;">'. $this->lang->line('true').'</label>
                <div class="col-md-6 col-sm-6 col-xs-12" style="display:inline-table"> 
