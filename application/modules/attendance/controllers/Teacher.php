    ...
    * ********************************************************** */ 
    public function update_single_attendance() {
        ...
        if ($this->teacher->update('teacher_attendances', array($field => $status, 'modified_at'=>date('Y-m-d H:i:s')), $condition)) {
            notifications($this->input->post('teacher_id'), 'teacher', 'attendance', 'Attendance Update - '.$this->input->post('date'));
    ...
    * ********************************************************** */
    public function update_all_attendance() {
        ...
        if ($this->teacher->update('teacher_attendances', array($field => $status, 'modified_at'=>date('Y-m-d H:i:s')), $condition)) {
            ...
            $allteachers =  $this->db->get_where('teacher_attendances', $condition )->result();
           
            foreach($allteachers as $keys =>  $vls){                
                notifications($vls->teacher_id, 'teacher', 'attendance', 'Attendance Update - '.strtotime($this->input->post('date')));
            }
