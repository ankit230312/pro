...
class Student extends MY_Controller {
    ...
    * ********************************************************** */ 
    ...
    public function update_single_attendance() {
        ...
        if ($this->student->update('student_attendances', array($field => $status, 'modified_at'=>date('Y-m-d H:i:s')), $condition)) {
            notifications($this->input->post('student_id'), 'student', 'attendance', 'Attendance Update - '.strtotime($this->input->post('date')));
            echo TRUE;
        } else {
            echo FALSE;
        }
    ...
    * ********************************************************** */
    ...
    public function update_all_attendance() {
        ...
        $field = 'day_' . abs(date('d', strtotime($this->input->post('date'))));
        if ($this->student->update('student_attendances', array($field => $status, 'modified_at'=>date('Y-m-d H:i:s')), $condition)) {
           $allstudents =  $this->db->get_where('student_attendances', $condition )->result();

           foreach($allstudents as $keys=>  $vls){
             notifications($vls->student_id, 'student', 'attendance', 'Attendance Update - '.strtotime($this->input->post('date')));
           }
