<div class="row">
                    ...
                    <ul  class="nav nav-tabs bordered">
                        ...
                        <li class="li-class-list">
                           ...
                           <?php if($this->session->userdata('role_id') == SUPER_ADMIN){  ?>
                                    ...
                                    <select  class="form-control col-md-7 col-xs-12" style="width:auto;" name="school_id"  onchange="get_route_by_school(this.value, '');">
                            ...
                            <?php }else{  ?>
                                ...
                                <select  class="form-control col-md-7 col-xs-12" id="class_id_filter" name="class_id" onchange="this.form.submit();">
                                    ...
                                    <?php foreach($class_list as $obj ){ ?>
                                        ...
                                        <?php foreach($route_list as $obj ){ ?>
                                        <?php if($this->session->userdata('role_id') == ADMIN){ ?>
                                           
                                            <option value="<?php echo $obj->id; ?>" <?php if(isset($class_id) && $class_id == $obj->id){ echo 'selected="selected"';} ?> ><?php echo $obj->title; ?></option>
                                        <?php } ?>                                            
                                    <?php } ?>     
 ...
 <script type="text/javascript">
    ...
    function get_route_by_school(school_id , class_id){
        $.ajax({       
            type   : "POST",
            url    : "<?php echo site_url('ajax/get_route_by_school'); ?>",
            data   : { school_id:school_id, class_id:class_id},               
            async  : false,
            success: function(response){                                                   
               if(response)
    ...
    <?php if(isset($filter_school_id) && isset($class_id)  && $this->session->userdata('role_id') == SUPER_ADMIN){ ?>
        get_route_by_school('<?php echo $filter_school_id; ?>', '<?php echo $class_id; ?>');
    <?php } ?>
