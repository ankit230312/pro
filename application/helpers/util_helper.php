...
if (!function_exists('get_slug')) {
    ...
    function get_slug($slug) {
        ...
        return $str. '-'. substr(str_shuffle("0123456789abcdefghijklmnopqrstvwxyzABCDEFGHIJKLMNOPQRSTVWXYZ"), 0, 5);
    }

}

if (!function_exists('get_slug1')) {

    function get_slug1($slug) {
        if (!$slug) {
            return;
        }
...
if (!function_exists('get_routines_by_day')) {
    ...
    function get_routines_by_day($day, $class_id = null, $section_id =null, $periods= null) {
        $ci = & get_instance();
...
if (!function_exists('get_period_routines')) {

    function get_period_routines($class_id = null, $section_id =null) {
        $ci = & get_instance();
        
        $ci->db->select('R.*, S.name AS subject, T.name AS teacher, C.name AS class_name');
        $ci->db->from('routines AS R');
        // $ci->db->group_by('R.day');
        $ci->db->group_by('R.period');
        $ci->db->join('subjects AS S', 'S.id = R.subject_id', 'left');
        $ci->db->join('teachers AS T', 'T.id = R.teacher_id', 'left');
        $ci->db->join('classes AS C', 'C.id = R.class_id', 'left');               
        if(logged_in_role_id() == TEACHER){
...
if (!function_exists('get_period_routines_by_school')) {
    function get_period_routines_by_school($school_id='null') {
        $ci = & get_instance();
...
if (!function_exists('get_exam_total_mark_online')) {

    function get_exam_total_mark_online($school_id, $exam_id, $student_id, $academic_year_id, $class_id, $section_id = null) {
        $ci = & get_instance();
        $ci->db->select('COUNT(M.id) AS total_subject, SUM(G.point) AS total_point, SUM(M.total_mark) AS exam_mark, SUM(M.total_obtain_mark) AS obtain_mark');
        $ci->db->from('exam_taken_exams AS M');        
        $ci->db->join('subjects AS S', 'S.id = M.subject_id', 'left');
        $ci->db->join('exam_online_exams AS ONE', 'ONE.id = M.exam_id', 'left');
        $ci->db->join('grades AS G', 'G.id = M.grade_id', 'left');
        $ci->db->where('M.school_id', $school_id);
        $ci->db->where('M.academic_year_id', $academic_year_id);
        $ci->db->where('M.class_id', $class_id);
        ...
        $ci->db->where('M.approve_status', 1);
        $ci->db->where('ONE.title', $exam_id);
        return  $ci->db->get()->row();     
    }
...
if (!function_exists('get_exam_result_multi_exams')) {

    function get_exam_result_multi_exams($school_id, $exim_id, $student_id, $academic_year_id, $class_id, $section_id = null) {
        $ci = & get_instance();
        $ci->db->select('SUM(ER.total_subject) AS total_subject,  SUM(ER.total_mark) AS total_mark,   SUM(ER.total_obtain_mark) AS total_obtain_mark,  SUM(ER.avg_grade_point) AS avg_grade_point');
        $ci->db->from('exam_results AS ER');  
        $ci->db->join('grades AS G', 'G.id = ER.grade_id', 'left');  
        $ci->db->where('ER.school_id', $school_id);
        $ci->db->where_in('ER.exam_id', $exim_id);
        $ci->db->where('ER.class_id', $class_id);
...
if (!function_exists('get_subject_list')) {
    ...
    function get_subject_list($school_id, $academic_year_id, $exam_id, $class_id, $section_id = null, $student_id = null) {
        $ci = & get_instance();
        $ci->db->select('M.*,S.name AS subject, G.point, G.name, Er.total_mark, Er.total_obtain_mark');
        $ci->db->from('marks AS M');        
        $ci->db->join('subjects AS S', 'S.id = M.subject_id', 'left');
        $ci->db->join('grades AS G', 'G.id = M.grade_id', 'left');
        $ci->db->join('exam_results AS Er', 'Er.exam_id = M.exam_id', 'left');
        $ci->db->where('M.school_id', $school_id);
        $ci->db->where('M.academic_year_id', $academic_year_id);
        $ci->db->where('M.class_id', $class_id);
        if($section_id){
...
if (!function_exists('get_subject_list_online')) {

    function get_subject_list_online($school_id, $academic_year_id, $exam_id, $class_id, $section_id = null, $student_id = null) {
        $ci = & get_instance();
        $ci->db->select('M.*,S.name AS subject, G.point, G.name');
        $ci->db->from('exam_taken_exams AS M');        
        $ci->db->join('subjects AS S', 'S.id = M.subject_id', 'left');
...
if (!function_exists('get_exam_wise_markt_online')) {

    function get_exam_wise_markt_online($school_id, $academic_year_id, $exam_id, $class_id, $section_id = null, $student_id = null) {
        $ci = & get_instance();
...
if (!function_exists('get_exam_detail_by_id')) {
    
    function get_exam_detail_by_id($exan_id){
        
        $ci = & get_instance();
        $ci->db->select('EX.*, EXT.title as exam_type_title, EXST.title as exam_sub_type_title');
        $ci->db->join('exam_types AS EXT', 'EXT.id = EX.exam_type_id', 'left');
        $ci->db->join('exam_sub_types AS EXST', 'EXST.id = EX.exam_sub_type_id', 'left');
        $ci->db->from('exams AS EX');  
        $ci->db->where('EX.id', $exan_id);
        return $ci->db->get()->row();       
    }
}

if (!function_exists('get_exam_online_exams_detail_by_id')) {
    
    function get_exam_online_exams_detail_by_id($online_exams){
        
        $ci = & get_instance();
        $ci->db->select('EX.*');
        $ci->db->from('exam_online_exams AS EX');  
        $ci->db->where('EX.id', $online_exams);
        return $ci->db->get()->row();       
    }
}
...
if (!function_exists('get_notification_message_student')) {

    function get_notification_message_student() {
       
            $ci = & get_instance();
            $ci->db->select('NT.*');
            $ci->db->from('notifications AS NT');
            $ci->db->join('students AS S', 'S.id = NT.to_user_id', 'left');
            $ci->db->join('users AS U', 'U.id = S.user_id', 'left');
            $ci->db->where('NT.is_read', 0);
            $ci->db->where('U.id', logged_in_user_id());
            $ci->db->order_by('NT.id', 'desc');
            return $ci->db->get()->result();
...
if (!function_exists('get_notification_message_guardian')) {

    function get_notification_message_guardian() {
            ...
            $ci->db->join('users AS U', 'U.id = S.user_id', 'left');
            $ci->db->join('guardians AS G', 'G.id = S.guardian_id', 'left');
            $ci->db->where('NT.is_read_guardian', 0);
            $ci->db->where('G.user_id', logged_in_user_id());
            $ci->db->order_by('NT.id', 'desc');
            return $ci->db->get()->result();
...
if (!function_exists('check_permission')) {
    ...
    function check_permission($action) {
        ...
        $modules = $ci->db->get_where('modules',['module_slug' =>$module_slug])->row();
        $checkpermission = 0;
        if ($modules) {
            $operations = $ci->db->get_where('operations',['operation_slug' =>$operation_slug, 'module_id' =>$modules->id])->row();
            if($operations){
                if ($action == 0) {
                    $privileges = $ci->db->get_where('privileges',['operation_id' =>$operations->id, 'role_id' =>$role_id, 'is_add'=> 1])->row();
                    if ($privileges) {
                        $checkpermission = 1;
                    }
                }elseif ($action == 1) {
                    $privileges = $ci->db->get_where('privileges',['operation_id' =>$operations->id, 'role_id' =>$role_id, 'is_edit'=> 1])->row();
                    if ($privileges) {
                        $checkpermission = 1;
                    }
                }elseif ($action == 2) {
                    $privileges = $ci->db->get_where('privileges',['operation_id' =>$operations->id, 'role_id' =>$role_id, 'is_view'=> 1])->row();
                    if ($privileges) {
                        $checkpermission = 1;
                    }
                }elseif ($action == 3) {
                    $privileges = $ci->db->get_where('privileges',['operation_id' =>$operations->id, 'role_id' =>$role_id, 'is_delete'=> 1])->row();
                    if ($privileges) {
                        $checkpermission = 1;
                    }
                }
...
if (!function_exists('notifications')) {
    ...
    function notifications($to_user_id, $to_type, $to_notification_type, $activity = null) {
        ...
        //$stdata = $ci->db->get_where('students',['id'=>$to_user_id])->row();
        // echo $ci->db->last_query();
        // print_r( $stdata);
        // die;

        $data['to_user_id'] = $to_user_id;
        $data['to_notification_type'] = $to_notification_type;
...
if (!function_exists('get_teacher_access_data_subjects')) {
    
    function get_teacher_access_data_subjects($type = NULL){
