        ...
        <nav>
            ...
            <div class="col-md-4">
                <ul class="nav navbar-nav <?php echo $this->enable_rtl ? 'navbar-left' : 'navbar-right'; ?>">
                    ...
                    <?php $notification_messages = get_notification_message_student();  ?>
                    
                    <li role="presentation" class="dropdown">
                        <a href="javascript:;" class="dropdown-toggle info-number " data-toggle="dropdown" aria-expanded="false">
                            <i class="fa fa-envelope-o"></i>
                            <span class="badge bg-green clickreadnotification" onclick="clickreadnotification()"><?php echo count($notification_messages); ?></span>
                        </a>
                        <ul id="menu1" class="dropdown-menu list-unstyled msg_list" role="menu">
                        <?php if(isset($notification_messages) && !empty($notification_messages)){ ?> 
                           <?php foreach($notification_messages as $obj){
                            // print_r( $obj);
                            ...
                            <li>
                                <?php 
                                $urls = '#';
                                if ($obj->to_notification_type == 'assignment') {
                                    $urls =  base_url().'academic/assignment/index';
                                }


                                if ($obj->to_notification_type == 'attendance') {
                                    $urls =  base_url().'profile/index';
                                }


                                if ($obj->to_notification_type == 'holiday') {
                                    $urls =  base_url().'announcement/holiday/index';
                                }


                                if ($obj->to_notification_type == 'generateticket') {
                                    $urls = '#';
                                }

                                if ($obj->to_notification_type == 'online_exams') {
                                    $urls =  base_url().'onlineexam/takeexam/index';
                                }

                                if ($obj->to_notification_type == 'in_app_messages') {
                                    $urls =  base_url().'message/inbox';
                                }
                            ...
                            <li>
                                <div class="text-center">                                    
                                        <strong>There are currently no notifications to display </strong>
                                </div>
                            </li>
                            <?php } ?>
                        </ul>
                    ...
                    <?php $notification_messages = get_notification_message_guardian();  ?>

                    <li role="presentation" class="dropdown">
                        <a href="javascript:;" class="dropdown-toggle info-number " data-toggle="dropdown" aria-expanded="false">
                            <i class="fa fa-envelope-o"></i>
                            <span class="badge bg-green clickreadnotificationguardian" onclick="clickreadnotificationguardian()"><?php echo count($notification_messages); ?></span>
                        </a>
                        <ul id="menu1" class="dropdown-menu list-unstyled msg_list" role="menu">
                        <?php if(isset($notification_messages) && !empty($notification_messages)){ ?> 
                        <?php foreach($notification_messages as $obj){
...
<?php if(has_permission(VIEW, 'setting', 'globalsearch') || has_permission(VIEW, 'setting', 'sessionchange')){ ?> 
    ...
    <div class="x_panel jhioi" style="padding-bottom: 2px;margin: 0px;margin-top: 70px;" >   
